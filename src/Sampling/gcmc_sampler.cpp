  ...
  gb_workspace_{nullptr}
{
  ...
  if (topology_->getUnitCellType() != UnitCellType::NONE) {
    ...
    ps_synthesis_ = new PhaseSpaceSynthesis(ps_by_value, ps_topo_vec,
                                            1, 1, 1, 1);  // Default cache parameters
    ...
    cell_grid_ = new CellGrid<double, double, double, double4>(
...
void GCMCSampler::adjustMoleculeLambda(GCMCMolecule& mol, double new_lambda) {
  ...
  // NOTE: Charges and LJ parameters are scaled during energy evaluation
  // via evaluateLambdaScaledNonbonded() using the lambda values stored here.
  // The topology itself remains unchanged (immutable).
}
...
double GCMCSampler::evaluateTotalEnergy() {
  ...
  for (const auto& mol : molecules_) {
    // DEBUG: Print lambda for first few ACTIVE molecules
  ...
  // Evaluate valence (bonded) energy for all atoms
  ...
  const double bond_energy = evaluateBondTerms(topology_, phase_space_, &scorecard_,
                                                EvaluateForce::NO, 0);
  const double angle_energy = evaluateAngleTerms(topology_, phase_space_, &scorecard_,
                                                  EvaluateForce::NO, 0);
  const double2 dihedral_energy = evaluateDihedralTerms(topology_, phase_space_, &scorecard_,
                                                         EvaluateForce::NO, 0);
  const double bonded_energy = bond_energy + angle_energy +
                                dihedral_energy.x + dihedral_energy.y;

  // Evaluate lambda-scaled nonbonded with pre-computed LJ parameters
  double2 nonbonded;

#ifdef STORMM_USE_CUDA
  // GPU path: use STORMM Hybrid arrays and PhaseSpace device pointers

  // Timing instrumentation
  static int energy_eval_count = 0;
  const bool enable_timing = (energy_eval_count % 10 == 0);  // Every 10th evaluation
  auto t_start = std::chrono::high_resolution_clock::now();
  auto t_setup_start = t_start;

  // Update Hybrid arrays with current lambda and LJ parameter values
  ...
  // Build list of coupled atom indices (atoms with lambda > threshold)
  ...
  if (n_coupled > 0) {
    ...
    // Upload output arrays to allocate device memory
    ...
    double* d_total_elec = total_elec_.data(HybridTargetLevel::DEVICE);
    double* d_total_vdw = total_vdw_.data(HybridTargetLevel::DEVICE);

    // Launch kernel with GPU-side reduction
    // Note: psw.xcrd/ycrd/zcrd point to GPU memory when CUDA is enabled
    ...
    energy::launchLambdaScaledNonbondedWithReduction(
        ...
        psw.xcrd, psw.ycrd, psw.zcrd, nbk.charge,
        d_lambda_vdw, d_lambda_ele,
        d_atom_sigma, d_atom_epsilon,
        ser.mask_data, ser.supertile_map_idx, ser.tile_map_idx, ser.supertile_stride_count,
        psw.umat, psw.unit_cell, nbk.coulomb_constant,
        ewald_coeff_,  // Ewald coefficient for PME direct space
        d_per_atom_elec, d_per_atom_vdw,
        d_total_elec, d_total_vdw);
    auto t_kernel_end = std::chrono::high_resolution_clock::now();

    // Download only scalar results (2 doubles instead of 2*n_coupled doubles!)
    auto t_download_start = std::chrono::high_resolution_clock::now();
    total_elec_.download();
    total_vdw_.download();
    auto t_download_end = std::chrono::high_resolution_clock::now();

    // Extract scalar totals (no CPU-side summation needed!)
    auto t_sum_start = std::chrono::high_resolution_clock::now();
    double elec_total = total_elec_.data()[0];
    double vdw_total = total_vdw_.data()[0];
    auto t_sum_end = std::chrono::high_resolution_clock::now();

    nonbonded.x = elec_total;
    nonbonded.y = vdw_total;

    // Print timing every 100 evaluations
    if (enable_timing) {
      double t_setup_ms = std::chrono::duration<double, std::milli>(t_setup_end - t_setup_start).count();
      double t_upload_ms = std::chrono::duration<double, std::milli>(t_upload_end - t_upload_start).count();
      double t_kernel_ms = std::chrono::duration<double, std::milli>(t_kernel_end - t_kernel_start).count();
      double t_download_ms = std::chrono::duration<double, std::milli>(t_download_end - t_download_start).count();
      double t_sum_ms = std::chrono::duration<double, std::milli>(t_sum_end - t_sum_start).count();
      double t_total_ms = std::chrono::duration<double, std::milli>(t_sum_end - t_start).count();

      printf("# ENERGY_TIMING (eval %d, n_coupled=%d): setup=%.3fms upload=%.3fms kernel=%.3fms download=%.3fms sum=%.3fms total=%.3fms\n",
             energy_eval_count, n_coupled, t_setup_ms, t_upload_ms, t_kernel_ms, t_download_ms, t_sum_ms, t_total_ms);
    }
  } else {
    nonbonded.x = 0.0;
    nonbonded.y = 0.0;
...
void GCMCSampler::applyRandomRotation(const GCMCMolecule& mol) {
  ...
  // Upload modified coordinates to GPU
...
bool GCMCSphereSampler::attemptInsertion() {
  ...
  if (rng_.uniformRandomNumber() < acc_prob) {
    ...
    adjustMoleculeLambda(*ghost_mol, 1.0);
...
bool GCMCSphereSampler::attemptDeletion() {
  ...
  adjustMoleculeLambda(*active_mol, 0.0);
  double E_final = evaluateTotalEnergy();
  ...
  if (rng_.uniformRandomNumber() < acc_prob) {
    ...
    adjustMoleculeLambda(*active_mol, 1.0);
...
void NCMCSampler::propagateSystem(int n_steps) {
  ...
  if (n_coupled > 0) {
    ...
    coupled_indices_.resize(n_coupled);
  ...
  phase_space_->uploadPositions();
    ...
    mm::MolecularMechanicsControls mmctrl(dt, n_steps);
    ...
    energy::CacheResource valence_cache(vale_lp.x, mm::maximum_valence_work_unit_atoms);
    ...
    energy::CacheResource nonb_cache(nonb_lp.x, synthesis::small_block_max_atoms);

    // Create static exclusion mask synthesis from the existing mask
    // This wraps the single system mask in a synthesis interface
    std::vector<energy::StaticExclusionMask*> mask_list = {exclusions_};
    std::vector<int> topology_indices = {0};  // Single system at index 0
    synthesis::StaticExclusionMaskSynthesis poly_se(mask_list, topology_indices);
    ...
    for (int step = 0; step < n_steps; step++) {
      ...
      mm::launchGpuLambdaDynamicsStep(
          n_atoms, n_coupled, d_coupled_indices,
          d_lambda_vdw, d_lambda_ele,
          d_atom_sigma, d_atom_epsilon,
          d_exclusion_mask, d_supertile_map, d_tile_map, supertile_stride,
          nbk.coulomb_constant,
          ewald_coeff_,  // Ewald coefficient for direct space erfc(α·r)/r splitting
          d_per_atom_elec, d_per_atom_vdw,
          d_xcrd, d_ycrd, d_zcrd,  // Raw device pointers for nonbonded kernel
          d_xfrc, d_yfrc, d_zfrc,  // Raw device pointers for nonbonded kernel
          d_umat, unit_cell,       // Unit cell info for nonbonded kernel
          nbk.charge,              // Charges for nonbonded kernel
          topology_synthesis_,     // Topology synthesis (constraints, virtual sites, valence)
          ps_synthesis_,           // Phase space synthesis (coordinates, velocities, forces)
          &poly_se,                // Static exclusion mask synthesis (for nonbonded interactions)
          ...
          &launcher,               // Kernel launcher for GPU parameters
...
double NCMCSampler::performNCMCProtocol(GCMCMolecule& mol, bool forward, bool propagate) {
  ...
  if (n_pert_steps == 0) {
    ...
    adjustMoleculeLambda(mol, lambda_values[1]);  // Jump directly to final lambda
  ...
  for (int i = 0; i < n_pert_steps; i++) {
    // Energy BEFORE lambda change (stored on GPU, not downloaded)
    energy::launchLambdaScaledNonbondedWithReduction(
        n_atoms, n_coupled, d_coupled_indices,
        psw.xcrd, psw.ycrd, psw.zcrd, nbk.charge,
        d_lambda_vdw, d_lambda_ele,
    ...
    // Update lambda on GPU (no CPU-side update, no upload!)
    // Pass VDW_COUPLING_THRESHOLD for two-stage coupling transformation
    ...
    // Energy AFTER lambda change (stored on GPU, not downloaded)
    energy::launchLambdaScaledNonbondedWithReduction(
        n_atoms, n_coupled, d_coupled_indices,
        psw.xcrd, psw.ycrd, psw.zcrd, nbk.charge,
        d_lambda_vdw, d_lambda_ele,
    ...
    // Accumulate work delta on GPU (no download!)
    energy::launchAccumulateWorkDelta(
        d_energy_before_elec, d_energy_before_vdw,
        d_energy_after_elec, d_energy_after_vdw,
        d_work);

    // Propagate at new lambda - call dynaStep directly to avoid coordinate synchronization
    if (propagate && n_prop_steps > 0) {
      try {
        for (int step = 0; step < n_prop_steps; step++) {
          dynaStep(&psw, &scorecard_, tstw, vk, nbk, isk, neck_gbk,
                   effective_gb_radii.data(), psi.data(), sumdeijda.data(),
                   rar, vsk, cdk, cnk, ser, dyn_controls_, 0);
        }
        // PBC wrapping not needed - energy kernels handle PBC correctly
      } catch (...) {
  ...
  // Download work ONCE at the end (no intermediate downloads!)
  work_accumulator_.download();
  work = work_accumulator_.data()[0];

  // Update CPU-side molecule lambda to match final GPU state (for consistency)
...
bool NCMCSampler::attemptInsertion() {
  ...
  const auto& lambdas = protocol_.getLambdaSchedule();
  adjustMoleculeLambda(*ghost_mol, lambdas[0]);

  // DEBUG: Verify lambda was set correctly
